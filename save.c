...
void				ft_getlens(t_file *head, char lens[][6])
{
	t_file			*ptr;
	char			tmp;
	...
	// 	tmp = 2;
		...
		if ((*lens)[3] < tmp)
			(*lens)[3] = tmp;
		if (S_ISCHR(ptr->filestat.st_size) || ((ptr->filestat.st_size & S_IFMT) == S_IFCHR))
		{
			ptr->major = ft_itoa(major(ptr->filestat.st_rdev));
			ptr->minor = ft_itoa(minor(ptr->filestat.st_rdev));
			tmp = ft_strlen(ptr->major) + ft_strlen(ptr->minor) + 2;
			tmp = 0;
			// dprintf(1, "ptr->path %s : major = %s, minor = %s, tmp = %i\n", ptr->path, ptr->major, ptr->minor, tmp);
		}
		else
			tmp = 0;
		if ((*lens)[5] < tmp)
...
void		ft_putfiles(t_file *head, int flags, t_times times)
{
	...
	if (flags & LL_FLAG && !(flags & UC_FLAG))
	{
		...
		dprintf(1, "SIZE 1 : %i\n", lens[5]);
		lens[4] = 11 + lens[0] + lens[1] + lens[2] + lens[3] + 7 + 13 + lens[5];
